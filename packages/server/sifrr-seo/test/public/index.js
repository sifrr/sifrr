(function () {
	...
	ab.activeElement),y(Window.prototype,sc,"__shady_"),t.g||(vc(),Yb()),Tb(),window.Event=Vb,window.CustomEvent=Wb,window.MouseEvent=Xb,window.ShadowRoot=P);}).call(commonjsGlobal);

	const temp = window.document.createElement('template');
	const script = window.document.createElement('script');
	const reg = '(\\${(?:(?:[^{}$]|{(?:[^{}$])*})*)})';
	const TEMPLATE = () => temp.cloneNode(false);
	const TREE_WALKER = () =>
	  window.document.createTreeWalker(window.document, window.NodeFilter.SHOW_ALL, null, false);
	const TEXT_NODE = 3;
	const COMMENT_NODE = 8;
	const ELEMENT_NODE = 1;
	const OUTER_REGEX = new RegExp(reg, 'g');
	...
	const addListener = (name, css, fxn) => {
