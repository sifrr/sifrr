...
describe('Sifrr.Dom.template', () => {
  ...
  it('works with bindings', async () => {
    expect((await getValue(() => Sifrr.Dom.template('abcd${ok}mnop'))).content).to.eq(
      'abcd${ok}mnop'
    );
    expect((await getValue(() => Sifrr.Dom.template`abcd\${ok}mnop`)).content).to.eq(
      'abcd${ok}mnop'
    ...
    expect(
      (await getValue(() => Sifrr.Dom.template(`abcd$\{ok}mnop`, 'div { width: 100% }'))).content
    ).to.eq('<style>div { width: 100% }</style>abcd${ok}mnop');
    expect((await getValue(() => Sifrr.Dom.template(document.createElement('p')))).content).to.eq(
      '<p></p>'
    );
    expect(
      (await getValue(() =>
