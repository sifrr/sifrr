    ...
    for (let i = 0; i < l; i++) {
      ...
      if (data.html) {
        ...
        if (Array.isArray(newValue)) {
          ...
          const docFrag = SIFRR_NODE.cloneNode(); // Replace html tags in input from input/contenteditable/textarea

          docFrag.innerHTML = newValue.toString() // All closing tags
...
class Loader {
  ...
  get html() {
    const me = this;
    return fetch.file(this.htmlUrl).then(resp => resp.text()).then(file => new window.DOMParser().parseFromString(file, 'text/html')).then(html => {
...
class Element extends window.HTMLElement {
  ...
  constructor() {
    ...
    this._refs = parser.collectRefs(content, this.constructor.stateMap); // this.useShadowRoot = this.constructor.template.dataset.sr === 'false' ? false : !!window.document.head.attachShadow && this.constructor.useShadowRoot;
    ...
    this.shadowRoot.addEventListener('change', parser.twoWayBind); // } else this.appendChild(content);
  }
